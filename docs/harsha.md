...
| ❌ | Sign off on `schemas/` by day 3 | Missed, and **not recoverable** — a deadline that has passed cannot be met later, and the contract R3 consumes was frozen without this seat's review. R1 and R2 record the same process failure. Five consequences are open with R4 and R2: the rollout row carries no prompt; R1's Parquet `extra` does not satisfy `rollout.schema.json`; the `generations/` vs `rollouts/` layer split is not in the schema; there is no entry point for replaying a **D1** policy; and the contract models two decision points where the project has three surfaces, so a probing router has nowhere to live. All five are the kind of thing a day-3 review is *for*. |
